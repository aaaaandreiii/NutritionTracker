...
import { 
  Settings, X, Info, CheckCircle2, Archive, Sparkles, Play, ShoppingCart, Plus, Edit2, ChevronDown 
} from 'lucide-react';
import { DAILY_DOZEN_CATEGORIES, foodDB, CategoryIcons, goalPresets, mealSlots } from '../data/constants';

export default function Dashboard({ 
  intake, setIntake, customTargets, setCustomTargets, isConfiguringTargets, 
  setIsConfiguringTargets, currentProgress, deficits, sortedDeficits, 
  pantry, setPantry, groceryList, setGroceryList, triggerToast, onOpenModal, handleClearMeal 
}) {
  ...
  const handleQuickLogFromPantry = (slotId, pantryItem) => {
    if (pantryItem.quantity <= 0) {
      triggerToast(`"${pantryItem.name}" is out of stock in your Pantry!`, 'warning');
      return;
    }
    ...
    triggerToast(`Logged "${pantryItem.name}" from stock to ${slotId}!`, 'success');
  };
  ...
  const handleAddToGrocery = (food) => {
    setGroceryList(prev => {
      ...
      return [...prev, { ...food, id: `g_food_${Date.now()}`, quantity: 1, checked: false }];
    });
  };

  return (
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
      {/* LEFT PORTFOLIO METRICS CHIP */}
      <section className="lg:col-span-7 space-y-8">
            ...
            <div className="flex justify-between items-center mb-4">
              <div>
                <h3 className="text-sm font-bold text-gray-800 flex items-center gap-1.5">
                  ...
                  Configure Metric Targets
                </h3>
                <p className="text-xs text-gray-400">Tweak Dr. Greger's base targets to personalize your dietary needs.</p>
              </div>
              <button onClick={() => setIsConfiguringTargets(false)} className="p-1 rounded-full text-gray-400 hover:bg-gray-100">
                <X size={16} />
              </button>
            </div>
                ...
                <div key={cat.id} className="p-2 border border-gray-50 rounded-xl bg-gray-50/50 flex flex-col justify-between">
                  ...
                  <div className="flex items-center gap-1.5 mt-1">
                    <input 
                      ...
                      className="w-14 text-xs font-bold px-2 py-1 bg-white border border-gray-200 rounded-md focus:outline-none"
                    />
        ...
        <div>
          <div className="flex justify-between items-end mb-4">
            <div>
              <h2 className="text-2xl font-serif text-gray-900 font-bold">Daily Progress Board</h2>
              <p className="text-xs text-gray-400">Aim to fill every segment by the end of the day. Click icons for details.</p>
            </div>
            <button 
              onClick={() => {
                setIntake({ breakfast: [], morningSnack: [], lunch: [], afternoonSnack: [], dinner: [] });
                triggerToast("Slate wiped clean!", "info");
              }}
              className="text-xs text-gray-400 hover:text-rose-500 flex items-center gap-1 font-semibold transition-colors"
            >
              <span>Reset Daily Log</span>
            </button>
          </div>
              ...
              return (
                ...
                >
                  ...
                  <div className="flex justify-between items-center text-[9px] text-gray-400 font-bold">
                    ...
                    {deficits[cat.id] > 0 && (
                      <span className="text-gray-400/90 font-medium">{deficits[cat.id].toFixed(1)} left</span>
                    )}
                  </div>

                  {infoPopup === cat.id && (
                    <div className="absolute inset-x-0 bottom-0 top-0 bg-white/95 rounded-3xl p-5 flex flex-col justify-between z-10 border border-gray-100 shadow-xl">
                      <div>
                        <div className="flex justify-between items-center mb-1.5">
                          <h5 className="text-xs font-bold text-gray-800 flex items-center gap-1.5">
                            <span className={cat.text}>{cat.label}</span>
                          </h5>
                          <button onClick={() => setInfoPopup(null)} className="text-gray-400 hover:text-gray-650">
                            <X size={14} />
                          </button>
                        </div>
                        <p className="text-[11px] text-gray-650 leading-relaxed font-serif italic mb-2">
                          "{cat.description}"
                        </p>
                      </div>
                      <button 
      ...
      {/* RIGHT DAILY SLOTS TIMELINE */}
      <section className="lg:col-span-5 space-y-8">
        <div>
          <h2 className="text-xl font-serif text-gray-900 font-bold mb-1">Your Daily Intake</h2>
          <p className="text-xs text-gray-400 mb-6">Log meals out-of-order, consume ingredients from your pantry, and get smart recommendations.</p>
          
          <div className="relative border-l-2 border-gray-100 ml-3.5 pl-8 space-y-8 py-2">
            {mealSlots.map((slot) => {
                  ...
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button 
                        ...
                        className="flex items-center gap-1 px-2.5 py-1 bg-white border border-gray-100 rounded-full hover:bg-gray-50 text-gray-500 hover:text-emerald-500 text-[11px] font-bold shadow-sm"
                      >
                        <Plus size={12} />
                        <span>Log</span>
                      </button>
                  ...
                  {hasLogged ? (
                    <div className="space-y-2 mb-3">
                      {loggedItems.map((item, idx) => {
                        ...
                        return (
                          <div key={idx} className="bg-white border border-gray-100 rounded-2xl p-3 flex justify-between items-center shadow-sm">
                            <div>
                              ...
                              <div className="flex flex-wrap gap-1 mt-1.5">
                                {Object.entries(item.servings || {}).map(([catId, amount]) => {
                                  const meta = DAILY_DOZEN_CATEGORIES.find(c => c.id === catId);
                            ...
                            <button 
                              onClick={() => onOpenModal(slot.id)}
                              className="p-1.5 text-gray-350 hover:text-gray-550"
                            >
                              <Edit2 size={12} />
                            </button>
                          </div>
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        {relevantPantryInStock.map(pItem => (
                          <button
                            ...
                            className="text-[10px] bg-white hover:bg-blue-5/50 border border-blue-100/40 text-gray-750 font-semibold px-2.5 py-1 rounded-xl flex items-center gap-1 transition-all"
                          >
                            <span>{pItem.name}</span>
                            <span className="text-[9px] text-blue-500 bg-blue-50 px-1 rounded-md">Qty: {pItem.quantity}</span>
                          </button>
                        ))}
                      </div>
                    </div>
                          ...
                          >
                            <div className="flex-1 pr-2">
                              <div className="flex items-center gap-1.5 flex-wrap">
                                ...
                                {sug.isStocked ? (
                                  <span className="bg-blue-50 text-blue-750 text-[8px] font-bold px-1.5 py-0.5 rounded-md">
                                    Pantry Stocked ({sug.pantryQty})
                                  </span>
                                ) : (
                            ...
                            <div className="flex items-center gap-1">
                              {sug.isStocked ? (
                                <button 
                                  ...
                                  className="p-1 bg-blue-100 text-blue-700 rounded-lg hover:bg-blue-550 transition-colors"
                                  title="Log from Pantry"
                                >
                                  <Play size={12} />
                                </button>
                              ) : (
                                <button 
                                  ...
                                  title="Add to shopping checklist"
                                >
