...
describe('mvp UI display helpers', () => {
  ...
  it('uses consumer labels for markets, review statuses, and scan state', () => {
    ...
    expect(scanSetupState({ hasEvidence: false, canAnalyze: false, analyzing: false, fails: 0, warnings: 0 }).label).toBe('Before evidence')
    expect(scanSetupState({ hasEvidence: true, canAnalyze: true, analyzing: false, fails: 0, warnings: 0 }).label).toBe('Evidence ready')
    expect(scanSetupState({ hasEvidence: true, canAnalyze: false, analyzing: false, fails: 1, warnings: 0 }).label).toBe('Needs attention')
    expect(scanSetupState({ hasEvidence: true, canAnalyze: false, analyzing: true, fails: 0, warnings: 0 }).label).toBe('Analysing')
  })
})
