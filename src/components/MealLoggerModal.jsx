...
import { DAILY_DOZEN_CATEGORIES, foodDB, mealSlots } from '../data/constants';

export default function MealLoggerModal({ isOpen, onClose, editingMeal, initialDraftItems = [], onSave }) {
  ...
  const [customCategoryServings, setCustomCategoryServings] = useState(
    DAILY_DOZEN_CATEGORIES.reduce((acc, cat) => ({ ...acc, [cat.id]: 0 }), {})
  );
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4 bg-black/45 backdrop-blur-sm">
      <div className="bg-white rounded-3xl w-full max-w-2xl overflow-hidden shadow-2xl flex flex-col max-h-[85vh]">
        <div className="p-6 border-b border-gray-100 bg-gray-50/50 flex justify-between items-center">
          <div>
            ...
            <p className="text-xs text-gray-400 mt-0.5">Edit portion sizes or define custom logs.</p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 divide-y md:divide-y-0 md:divide-x divide-gray-100 overflow-hidden flex-1">
            ...
            {!customItemMode ? (
              <>
                <div className="relative mb-4">
                  ...
                  <input 
                    ...
                    className="w-full bg-gray-50 border-none rounded-xl py-2 pl-10 pr-4 text-xs focus:ring-1 focus:ring-emerald-500/20 focus:bg-white outline-none"
                  />
                </div>
                    ...
                    .map(food => {
                      ...
                      return (
                        <div 
                          ...
                          className={`flex items-center justify-between p-3 rounded-xl cursor-pointer border transition-all duration-205 ${
                            isSelected ? 'border-emerald-200 bg-emerald-50/20 shadow-sm' : 'border-transparent hover:bg-gray-50'
                          }`}
                        >
                          <div>
                            ...
                            <span className="text-[10px] text-gray-400">
                              {food.cals} kcal • {Object.entries(food.servings).map(([cat, amt]) => {
                                const meta = DAILY_DOZEN_CATEGORIES.find(c => c.id === cat);
              ...
              <form onSubmit={handleAddCustomItemSubmit} className="space-y-4">
                <div>
                  ...
                  <input 
                    type="text" 
                    placeholder="e.g. Mixed Salad Bowl" 
                    value={customName}
                    onChange={(e) => setCustomName(e.target.value)}
                    required
                    className="w-full bg-gray-50 border border-gray-100 rounded-xl px-3 py-2 text-xs focus:ring-1 focus:ring-emerald-500"
                  />
                </div>
                ...
                <div>
                  <label className="block text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-2">Assign Servings</label>
                  <div className="grid grid-cols-2 gap-2 max-h-[18vh] overflow-y-auto pr-1">
                    {DAILY_DOZEN_CATEGORIES.map(cat => (
                      <div key={cat.id} className="flex items-center justify-between p-2 bg-gray-50/50 border border-gray-100 rounded-xl">
                        <span className="text-[10px] font-medium text-gray-650 truncate mr-2">{cat.label}</span>
                        <input 
                          type="number" 
                          step="0.5"
                          min="0"
                ...
                <button 
                  ...
                  className="w-full py-2 px-4 bg-emerald-500 text-white rounded-xl text-xs font-bold hover:bg-emerald-600 disabled:opacity-50 transition-colors"
                >
                  Log Custom Block
                </button>
              </form>
          ...
          <div className="md:col-span-5 p-6 bg-gray-50/30 flex flex-col overflow-y-auto max-h-[40vh] md:max-h-[60vh]">
            <h4 className="text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-3">Logs Config</h4>
            {draftItems.length === 0 ? (
              <div className="flex-1 flex flex-col items-center justify-center text-center py-8">
                <p className="text-xs text-gray-400 italic">Select ingredients from the directory.</p>
              </div>
            ) : (
              <div className="space-y-3 flex-1 overflow-y-auto">
                {draftItems.map((item, index) => (
                  <div key={item.id} className="bg-white border border-gray-100 rounded-xl p-3 shadow-sm relative">
                    <div className="flex justify-between items-start pr-4">
                      <div>
                        <span className="text-xs font-semibold text-gray-800 block leading-snug">{item.name}</span>
                        <span className="text-[10px] text-gray-450">{Math.round(item.cals * (item.servingsMultiplier || 1.0))} kcal total</span>
                      </div>
                      <button 
                        type="button"
        ...
        <div className="p-6 border-t border-gray-100 bg-gray-50/50 flex justify-between items-center">
          ...
          <div className="flex gap-2">
            <button 
              type="button"
              onClick={onClose}
              className="px-4 py-2 bg-white hover:bg-gray-50 text-gray-500 border border-gray-200 text-xs font-semibold rounded-full"
            >
              Cancel
            </button>
            <button 
              ...
              className="px-6 py-2 bg-emerald-500 hover:bg-emerald-600 text-white text-xs font-bold rounded-full"
            >
              Log Foods
            </button>
          </div>
