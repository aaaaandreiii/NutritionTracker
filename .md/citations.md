...
| `open-food-facts-local-export` | Open Food Facts product data export, transformed into `backend/app/data/off_ph_products.db` from `research/openfoodfacts_export.csv`. https://world.openfoodfacts.org/data | Barcode product identity, serving/nutrient draft evidence, ingredient text for taxonomy matching, and descriptive markers such as NOVA, Nutri-Score, and allergens. | Not used as medical advice, a rating, a glucose prediction, or user-confirmed label truth without finalize. |
...
| Local barcode database evidence | `GET /api/v1/off-products/{barcode}` and `POST /api/v1/analyses/barcode` use the generated SQLite database for database-sourced evidence values. | Open Food Facts community data is shown as unconfirmed `sourceKind: "database"` evidence and must be user-confirmed before logging. |
...
| `src/components/mvp/BarcodeScannerModal.tsx` | Live camera barcode detection using ZXing. |
...
| `backend/app/db/ingest_off.py` | Generates the local OFF SQLite product database. |
| `backend/app/db/off_products.py` | Local OFF SQLite lookup and response shaping. |
